...
// BUTON VE MENU CLASSLARI VERİLMİŞ BUNLARI ÇEKİP ATAMASINI YAPMA

let sectionDOM = document.querySelector('.section-center')
let btncontainerDOM = document.querySelector('.btn-container')

// BUTON TANIMLAMASI YAPIP BUTON EKLEME

function createButton(buttonName) {
    let btn = `<button type="button" class="btn btn-outline-dark btn-item" id="${buttonName}">${buttonName}</button> `
    return btn
}
...
window.addEventListener('load', addButton ) // Ekranda butonlarımız gözükür hala geldi
...
function addFoodAll() {
    ...
    menu.forEach(element => {
        sectionDOM.innerHTML += (" " + createFood(element))
    });
}
...
window.addEventListener('load',  addFoodAll) // Menunun ekranda gözükmesini sağlandı
